...
if ($('.header').hasClass('not-main-header')) {
    ...
    if ($(window).width()>=0){
        ...
        $('.header-form-wrap').addClass('fixed-faked-never-braked');
    }
}
...
$('.move-form').click(function(){
    if ( ($('.wsplivashka').hasClass('move-nigga'))) {
        $('.contact-form-in-foo').submit();
    };
    $('.wsplivashka').addClass('move-nigga');
...
$(document).click(function (e) {
    if ($('.loader-move').css('z-index')==0) {
        var container = $(".wsplivashka");
        if ( ($('.wsplivashka').hasClass('move-nigga'))) {
            if (($(".header-top-buttons-wrap").has(e.target).length == 0) && (container.has(e.target).length == 0)){
                 $('.wsplivashka').removeClass('move-nigga');
                 $('.footer_placeholder').height($('.footer').outerHeight());
                 setTimeout(function(){
                    $('.x-close').css('display','none');
